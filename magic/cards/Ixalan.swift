...
enum XLN {
    ...
    static func AxisOfMortality() -> Card {
        let axisOfMortality = Card(name: "Axis of Mortality", rarity: .Mythic, set: set, number: 3)
        axisOfMortality.setManaCost("4WW")
        axisOfMortality.setType(.Enchantment)
        axisOfMortality.addTriggeredAbility(
            trigger: .YourUpkeep,
            effect: TargetedEffect.MultiPlayer(
                restrictions: [TargetingRestriction.TargetPlayer(), TargetingRestriction.TargetPlayer()],
                effect: { targets in
                    if let player1 = targets[0], let player2 = targets[1] {
                        let temp = player1.getLife()
                        player1.setLife(player2.getLife())
                        player2.setLife(temp)
                    }
                },
                distinctTargets: true,
                effectOptional: true))
        axisOfMortality.setFlavorText("The Church of Dusk teaches there is no gain without loss, no triumph without sacrifice, no drink of exquisite crimson without a dying gasp.")
        return axisOfMortality
    }
    ...
    static func BishopOfRebirth() -> Card {
        let bishopOfRebirth = Card(name: "Bishop of Rebirth", rarity: .Rare, set: set, number: 5)
        bishopOfRebirth.setManaCost("3WW")
        bishopOfRebirth.setType(.Creature, .Vampire, .Cleric)
        bishopOfRebirth.vigilance = true
        bishopOfRebirth.addTriggeredAbility(
            trigger: .ThisAttacks,
            effect: TargetedEffect.SingleObject(
                restriction: TargetingRestriction.SingleObject(
                    restriction: { $0.isType(.Creature) && $0.getConvertedManaCost() <= 3 && $0.getOwner() === bishopOfRebirth.getOwner() },
                    zones: [.Graveyard]),
                effect: { $0.putOntoBattlefield() },
                effectOptional: true))
        bishopOfRebirth.setFlavorText("\"In the death of the foe lies the resurrection of the faithful.\"")
        bishopOfRebirth.power = 3
        bishopOfRebirth.toughness = 4
        return bishopOfRebirth
    }
    ...
    static func ImperialLancer() -> Card {
        ...
        imperialLancer.addStaticAbility(
            requirement: AbilityRequirement.This(imperialLancer),
            effect: { object in
    ...
    static func PterodonKnight() -> Card {
        ...
        pterodonKnight.addStaticAbility(
            requirement: AbilityRequirement.This(pterodonKnight),
            effect: { object in
                if !object.getController().getPermanents().filter({ return $0.isType(.Dinosaur) }).isEmpty {
                    object.flying = true
                }
                return object
        })
    ...
    static func Cancel() -> Card {
        let cancel = Card(name: "Cancel", rarity: .Common, set: set, number: 47)
    ...
    static func FavorableWinds() -> Card {
        ...
        favorableWinds.addStaticAbility(
    ...
    static func HeraldOfSecretStreams() -> Card {
        ...
        heraldOfSecretStreams.addStaticAbility(
            requirement: AbilityRequirement.CreaturesYouControl(
    ...
    static func ShaperApprentice() -> Card {
        ...
        shaperApprentice.addStaticAbility(
            requirement: AbilityRequirement.This(shaperApprentice),
            effect: { object in
                if !object.getController().getPermanents().filter({ return $0 != object && $0.isType(.Merfolk) }).isEmpty {
                    object.flying = true
                }
                return object
        })
    ...
    // 80 Siren's Ruse
    // 81 Spell Pierce
    static func SpellSwindle() -> Card {
        let spellSwindle = Card(name: "Spell Swindle", rarity: .Rare, set: set, number: 82)
        spellSwindle.setManaCost("3UU")
        spellSwindle.setType(.Instant)
        spellSwindle.addEffect(TargetedEffect.SingleObject(
            restriction: TargetingRestriction.TargetSpell(),
            effect: { target in
                let cmc = target.getConvertedManaCost()
                target.counter()
                for _ in 0 ..< cmc {
                    spellSwindle.getController().createToken(Treasure())
                }
        }))
        spellSwindle.setFlavorText("\"Hold your fire, obviously.\"\n--Captain Brandis Thorn")
        return spellSwindle
    }
        ...
        skulduggery.addEffect(TargetedEffect.MultiObject(
            restrictions: [
                TargetingRestriction.SingleObject(
                    restriction: { $0.isType(.Creature) && $0.getController() === skulduggery.getController() },
                    zones: [.Battlefield]
                ),
                TargetingRestriction.SingleObject(
                    restriction: { $0.isType(.Creature) && $0.getController() !== skulduggery.getController() },
                ...
                restrictions: [
                    TargetingRestriction(
                        restriction: { targetable in
                            if let player = targetable as? Player {
                                return player !== burningSunsAvatar.getController()
                            }
                            else if let object = targetable as? Object {
            ...
            effect: { targets in
                for target in targets {
                    if let actualTarget = target {
                        dualShot.damage(to: actualTarget, 1)
                    }
                }
            },
            distinctTargets: true))
    ...
    static func AtzocanArcher() -> Card {
        let atzocanArcher = Card(name: "Atzocan Archer", rarity: .Uncommon, set: set, number: 176)
        atzocanArcher.setManaCost("2G")
        atzocanArcher.setType(.Creature, .Human, .Archer)
        atzocanArcher.reach = true
        atzocanArcher.addTriggeredAbility(
            trigger: .ThisETB,
            effect: TargetedEffect.SingleObject(
                restriction: TargetingRestriction.SingleObject(
                    restriction: { $0 != atzocanArcher && $0.isType(.Creature) },
                    zones: [.Battlefield]),
                effect: { atzocanArcher.fight($0) },
                effectOptional: true))
        atzocanArcher.setFlavorText("His arrows are fletched with feathers from the greatest of the winged dinosaurs.")
        atzocanArcher.power = 1
        atzocanArcher.toughness = 4
        return atzocanArcher
    }
    // 177 Blinding Fog
    ...
    static func CarnageTyrant() -> Card {
        let carnageTyrant = Card(name: "Carnage Tyrant", rarity: .Mythic, set: set, number: 179)
        carnageTyrant.setManaCost("4GG")
        carnageTyrant.setType(.Creature, .Dinosaur)
        carnageTyrant.uncounterable = true
        carnageTyrant.trample = true
        carnageTyrant.hexproof = true
        carnageTyrant.setFlavorText("Sun Empire commanders are well versed in advanced martial strategy. Still, the correct maneuver is usually to deploy the giant, implacable death lizard.")
        carnageTyrant.power = 7
        carnageTyrant.toughness = 6
        return carnageTyrant
    }
    ...
    static func DroverOfTheMighty() -> Card {
        ...
        droverOfTheMighty.addStaticAbility(
            ...
            effect: { object in
                if !object.getController().getPermanents().filter({ $0.isType(.Dinosaur) }).isEmpty {
                    object.pump(2, 2)
                }
                return object
        })
    ...
    static func KumenasSpeaker() -> Card {
        ...
        kumenasSpeaker.addStaticAbility(
            requirement: AbilityRequirement.This(kumenasSpeaker),
            effect: { object in
    ...
    static func Pounce() -> Card {
        ...
        pounce.addEffect(TargetedEffect.MultiObject(
            restrictions: [
                TargetingRestriction.SingleObject(
                    restriction: { $0.isType(.Creature) && $0.getController() === pounce.getController() },
                    zones: [.Battlefield]),
                TargetingRestriction.SingleObject(
                    restriction: { $0.isType(.Creature) && $0.getController() !== pounce.getController() },
                    zones: [.Battlefield])
    ...
    static func DeadeyePlunderers() -> Card {
        ...
        deadeyePlunderers.addStaticAbility(
            requirement: AbilityRequirement.This(deadeyePlunderers),
            effect: { object in
                let numArtifacts = object.getController().getArtifacts().count
                object.pump(numArtifacts, numArtifacts)
                return object
        })
    ...
    static func DragonskullSummit() -> Card {
        ...
        dragonskullSummit.addStaticAbility(
            requirement: AbilityRequirement.This(dragonskullSummit),
            effect: { object in
    ...
    static func DrownedCatacomb() -> Card {
        ...
        drownedCatacomb.addStaticAbility(
            requirement: AbilityRequirement.This(drownedCatacomb),
            effect: { object in
    ...
    static func GlacialFortress() -> Card {
        ...
        glacialFortress.addStaticAbility(
            requirement: AbilityRequirement.This(glacialFortress),
            effect: { object in
    ...
    static func RootboundCrag() -> Card {
        ...
        rootboundCrag.addStaticAbility(
            requirement: AbilityRequirement.This(rootboundCrag),
            effect: { object in
                let controlsMountain = !object.getController().getLands().filter({ $0.isType(.Mountain) }).isEmpty
    ...
    static func SunpetalGrove() -> Card {
        ...
        sunpetalGrove.addStaticAbility(
            requirement: AbilityRequirement.This(sunpetalGrove),
            effect: { object in
                let controlsForest = !object.getController().getLands().filter({ $0.isType(.Forest) }).isEmpty
                let controlsPlains = !object.getController().getLands().filter({ $0.isType(.Plains) }).isEmpty
