...
enum M19 {
    ...
    static func AjaniAdversaryOfTyrants() -> Card {
        let ajani = Card(name: "Ajani, Adversary of Tyrants", rarity: .Mythic, set: set, number: 3)
        ajani.setManaCost("2WW")
        ajani.setType(.Legendary, .Planeswalker, .Ajani)
        ajani.addActivatedAbility(
            string: "{+1}: Put a +1/+1 counter on each of up to two target creatures.",
            ...
            effect: TargetedEffect.MultiObject(
                ...
                effect: { targets in
                    targets.forEach({ $0?.addCounter(.PlusOnePlusOne) })
                },
            ...
            loyaltyAbility: true)
        ajani.addActivatedAbility(
            string: "{-2}: Return target creature card with converted mana cost 2 or less from your graveyard to the battlefield.",
            cost: Cost.RemoveCounters(.Loyalty, 2),
            effect: TargetedEffect.SingleObject(
                restriction: TargetingRestriction.SingleObject(
                    restriction: { $0.isType(.Creature) && $0.getConvertedManaCost() <= 2 && $0.getOwner() === ajani.getOwner() },
                    zones: [.Graveyard]
                ),
            ...
            loyaltyAbility: true)
        ajani.addActivatedAbility(
            string: "{-7}: You get an emblem with \"At the beginning of your end step, create three 1/1 white Cat creature tokens with lifelink.\"",
            cost: Cost.RemoveCounters(.Loyalty, 7),
            effect: { ajani.getController().createEmblem(AjaniAdversaryOfTyrantsEmblem()) },
            manaAbility: false,
            sorcerySpeed: true,
            loyaltyAbility: true)
        ajani.startingLoyalty = 4
        return ajani
    }
    ...
    static func AngelOfTheDawn() -> Card {
        ...
        angelOfTheDawn.addTriggeredAbility(
            ...
            effect: { angelOfTheDawn.getController().getCreatures().forEach({
                $0.pump(1, 1)
                $0.giveKeywordUntilEndOfTurn(.Vigilance)
            })
        })
    ...
    static func MightyLeap() -> Card {
        ...
        mightyLeap.addEffect(TargetedEffect.SingleObject(
            ...
            effect: {
                $0.pump(2, 2)
                $0.giveKeywordUntilEndOfTurn(.Flying)
        }))
    ...
    static func ResplendentAngel() -> Card {
        ...
        resplendentAngel.addActivatedAbility(
            ...
            effect: {
                ...
                resplendentAngel.giveKeywordUntilEndOfTurn(.Lifelink)
        })
    ...
    static func ValiantKnight() -> Card {
        ...
        valiantKnight.addActivatedAbility(
            ...
            effect: { valiantKnight.getController().getPermanents().filter({ $0.isType(.Knight) }).forEach({ knight in
                knight.giveKeywordUntilEndOfTurn(.DoubleStrike)
            })
        })
    ...
    static func AetherTunnel() -> Card {
        ...
        aetherTunnel.addEnchantAbility(
            restriction: TargetingRestriction.TargetCreature(),
            effects: [
                ({ return $0.pumped(1, 0) }, .PowerToughnessChanging),
                ({ return $0.withKeyword(.Unblockable) }, .AbilityAddingOrRemoving)
            ])
    ...
    static func BoneToAsh() -> Card {
        let boneToAsh = Card(name: "Bone to Ash", rarity: .Uncommon, set: set, number: 47)
    ...
    static func Cancel() -> Card {
        let cancel = Card(name: "Cancel", rarity: .Common, set: set, number: 48)
        cancel.setManaCost("1UU")
        cancel.setType(.Instant)
        cancel.addEffect(TargetedEffect.SingleObject(
            restriction: TargetingRestriction.TargetSpell(),
            effect: { $0.counter() }
        ))
        cancel.setFlavorText("\"Hold your fire, obviously.\"\n--Captain Brandis Thorn")
        return cancel
    }
    ...
    // 52 Djinn of Wishes
    // 53 Dwindle
    static func EssenceScatter() -> Card {
        let essenceScatter = Card(name: "Essence Scatter", rarity: .Common, set: set, number: 54)
        essenceScatter.setManaCost("1U")
        essenceScatter.setType(.Instant)
        essenceScatter.addEffect(TargetedEffect.SingleObject(
            restriction: TargetingRestriction.TargetCreatureSpell(),
            effect: { $0.counter() }))
        essenceScatter.setFlavorText("Dependence on luck is anathema to Kefnet's rigorous studies. Those who hope to escape his maze by chance never succeed.")
        return essenceScatter
    }
    ...
    static func Ghostform() -> Card {
        let ghostform = Card(name: "Ghostform", rarity: .Common, set: set, number: 58)
        ghostform.setManaCost("1U")
        ghostform.setType(.Sorcery)
        ghostform.addEffect(TargetedEffect.MultiObject(
            ...
            distinctTargets: true))
        ghostform.setFlavorText("\"Picking locks is for beginners.\"")
        return ghostform
    }
    // 59 Horizon Scholar
    ...
    static func SalvagerOfSecrets() -> Card {
        let salvagerOfSecrets = Card(name: "Salvager of Secrets", rarity: .Common, set: set, number: 70)
        salvagerOfSecrets.setManaCost("3UU")
        salvagerOfSecrets.setType(.Creature, .Merfolk, .Wizard)
        salvagerOfSecrets.addTriggeredAbility(
            trigger: .ThisETB,
            effect: TargetedEffect.SingleObject(
                restriction: TargetingRestriction.SingleObject(
                    restriction: { ($0.isType(.Instant) || $0.isType(.Sorcery)) && $0.getOwner() === salvagerOfSecrets.getController() },
                    zones: [.Graveyard]),
                effect: { $0.putIntoHand() }))
        salvagerOfSecrets.setFlavorText("The greatest treasure of the deep is an echo from above.")
        salvagerOfSecrets.power = 2
        salvagerOfSecrets.toughness = 2
        return salvagerOfSecrets
    }
    ...
    static func DeathBaron() -> Card {
        ...
        deathBaron.addStaticAbility(
            ...
            effects: [
                ...
                ({ return $0.withKeyword(.Deathtouch) }, .AbilityAddingOrRemoving)
            ])
    ...
    // 112 Open the Graves
    // 113 Phylactery Lich
    static func PlagueMare() -> Card {
        let plagueMare = Card(name: "Plague Mare", rarity: .Uncommon, set: set, number: 114)
        plagueMare.setManaCost("1BB")
        plagueMare.setType(.Creature, .Nightmare, .Horse)
        plagueMare.blockabilityRequirements.append({ object in
            return !(object.isColor(.White) && object.isType(.Creature))
        })
    ...
    static func LavaAxe() -> Card {
        let lavaAxe = Card(name: "Lava Axe", rarity: .Common, set: set, number: 150)
        lavaAxe.setManaCost("4R")
        lavaAxe.setType(.Sorcery)
        lavaAxe.addEffect(TargetedEffect(
            restriction: TargetingRestriction.TargetPlayerOrPlaneswalker(),
            effect: { lavaAxe.damage(to: $0, 5) }))
        lavaAxe.setFlavorText("\"Catch!\"")
        return lavaAxe
    }
    static func LightningMare() -> Card {
        let lightningMare = Card(name: "Lightning Mare", rarity: .Uncommon, set: set, number: 151)
        lightningMare.setManaCost("RR")
        lightningMare.setType(.Creature, .Elemental, .Horse)
        lightningMare.uncounterable = true
        lightningMare.blockabilityRequirements.append({ object in
            return !(object.isColor(.Blue) && object.isType(.Creature))
        })
        lightningMare.addActivatedAbility(
            string: "{1}{R}: ~ gets +1/+0 until end of turn.",
            cost: Cost.Mana("1R"),
            effect: { lightningMare.pump(1, 0) })
        lightningMare.setFlavorText("When it passes, storm clouds bolt across the land.")
        lightningMare.power = 3
        lightningMare.toughness = 1
        return lightningMare
    }
    ...
    static func ViashinoPyromancer() -> Card {
        let viashinoPyromancer = Card(name: "Viashino Pyromancer", rarity: .Common, set: set, number: 166)
        viashinoPyromancer.setManaCost("1R")
        viashinoPyromancer.setType(.Creature, .Viashino, .Wizard)
        viashinoPyromancer.addTriggeredAbility(
            trigger: .ThisETB,
            effect: TargetedEffect.SingleObject(
                restriction: TargetingRestriction.TargetPlayerOrPlaneswalker(),
                effect: { viashinoPyromancer.damage(to: $0, 2) }))
        viashinoPyromancer.setFlavorText("\"I trained with the Ghitu, and I respect the fire as they do.\"")
        viashinoPyromancer.power = 2
        viashinoPyromancer.toughness = 1
        return viashinoPyromancer
    }
    ...
    static func GoreclawTerrorOfQalSisma() -> Card {
    ...
    static func ProdigiousGrowth() -> Card {
        ...
        prodigiousGrowth.addEnchantAbility(
            restriction: TargetingRestriction.TargetCreature(),
            effects: [
                ({ return $0.pumped(7, 7) }, .PowerToughnessChanging),
    ...
    static func Recollect() -> Card {
        ...
        recollect.addEffect(TargetedEffect.SingleObject(
            restriction: TargetingRestriction.SingleObject(
                restriction: { $0.getOwner() === recollect.getOwner() },
                zones: [.Graveyard] ),
            effect: { $0.putIntoHand() }))
        recollect.setFlavorText("\"The bones of the past will tell their tales--if you know how to speak their language.\"")
        return recollect
    }
    ...
    static func VineMare() -> Card {
        let vineMare = Card(name: "Vine Mare", rarity: .Uncommon, set: set, number: 207)
        vineMare.setManaCost("2GG")
        vineMare.setType(.Creature, .Elemental, .Horse)
        vineMare.hexproof = true
        vineMare.blockabilityRequirements.append({ object in
            return !(object.isColor(.Black) && object.isType(.Creature))
        })
        vineMare.setFlavorText("When it passes, the dead are displaced by flourishing life.")
        vineMare.power = 5
        vineMare.toughness = 3
        return vineMare
    }
    ...
    static func HeroicReinforcements() -> Card {
        ...
        heroicReinforcements.addEffect {
            ...
            heroicReinforcements.getController().getCreatures().forEach({ creature in
                creature.pump(1, 1)
                creature.giveKeywordUntilEndOfTurn(.Haste)
            })
        }
    ...
    static func PalladiaMorsTheRuiner() -> Card {
        ...
        palladiaMors.addStaticAbility(
            ...
            effect: { return $0.hasDealtDamage ? $0 : $0.withKeyword(.Hexproof) },
    ...
    static func GargoyleSentinel() -> Card {
        ...
        gargoyleSentinel.addActivatedAbility(
            ...
            cost: Cost.Mana("3"),
            effect: {
                gargoyleSentinel.removeKeywordUntilEndOfTurn(.Defender)
                gargoyleSentinel.giveKeywordUntilEndOfTurn(.Flying)
        })
    ...
    static func AjaniAdversaryOfTyrantsEmblem() -> Object {
        let ajaniEmblem = Emblem(set: set, number: 15)
        ajaniEmblem.addTriggeredAbility(
            trigger: .YourEndStep,
            effect: {
                for _ in 0 ..< 3 {
                    ajaniEmblem.getController().createToken(Cat())
                }
        })
        return ajaniEmblem
    }
    ...
    static func VivienReidEmblem() -> Object {
        ...
        vivienEmblem.addStaticAbility(
            ...
            effects: [
                ({ return $0.withKeyword(.Vigilance) }, .AbilityAddingOrRemoving),
                ({ return $0.withKeyword(.Trample) }, .AbilityAddingOrRemoving),
                ({ return $0.withKeyword(.Indestructible) }, .AbilityAddingOrRemoving),
            ])
        return vivienEmblem
    }
}
